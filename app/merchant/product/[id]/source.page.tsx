...
import { ProductRequest, Product } from '@/model/product'
import InputLabel from '@/components/label/label'
import { patchProduct } from '@/redux/merchant/product/action'
...
const EditProductSourcePage: React.FC<IProps> = ({ data }) => {
    ...
    const onSubmit = () => {
        ...
        dispatch(patchProduct({ req: productRequest })).then(response => {
    ...
    const initialPage = useCallback(() => {
        ...
        dispatch(SetVehicleNumber(data.VehicleNumber))
...
export default EditProductSourcePage
