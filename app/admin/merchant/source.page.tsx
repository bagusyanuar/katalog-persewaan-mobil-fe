...
import {
    AdminMerchantState,
    SetLoadingMerchant
} from '@/redux/admin/merchant/slice'
import { getMerchantList } from '@/redux/admin/merchant/action'
...
const DriverSourcePage: React.FC = () => {
    const StateAdminMerchant = useAppSelector(AdminMerchantState)
    const dispatch = useAppDispatch()
    const router = useRouter()
    const [modal, setModal] = useState<boolean>(false)

    const columns: TableColumn<AdminMerchant>[] = [
        {
            name: 'Email',
    ...
    return (
        <Wrapper >
            ...
            <ContentWrapper>
                ...
                <ContentContainer>
                    <HeaderContainer>
                        <HeaderTitle>Halaman Merchant</HeaderTitle>
                        <BreadcrumbWrapper>
                            <a href='/admin/dashboard'>Dashboard</a>
                            <span>/</span>
                            <a href='#' className='active'>Merchant</a>
                        </BreadcrumbWrapper>
                    </HeaderContainer>

                    <CardContent>
                        <div className='flex justify-between items-center w-full'>
                            <DataTitle>Data Merchant</DataTitle>
                        </div>
                        <hr className='my-5' />
                        <DataTable
                            columns={columns}
                            data={StateAdminMerchant.Merchants}
                            pagination
                            progressPending={StateAdminMerchant.LoadingMerchant}
